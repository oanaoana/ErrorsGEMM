...
# Force a draw so ticklabels exist as Text objects
...
# Second plot: E_{AB}/beta vs matrix size
plt.figure(figsize=FIGURE_SIZE)
for label, kernels_data in data.items():
    for kernel, df in kernels_data.items():
...
# Third plot: E_{AB}/u_c vs matrix size
plt.figure(figsize=FIGURE_SIZE)
for label, kernels_data in data.items():
    for kernel, df in kernels_data.items():
...
# Fourth plot: E_{AB}/u_a vs matrix size
plt.figure(figsize=FIGURE_SIZE)
for label, kernels_data in data.items():
    for kernel, df in kernels_data.items():
        if not df.empty and 'E_{AB}/u_a' in df.columns:
            plt.plot(
                df['size'], df['E_{AB}/u_a'],
                marker=MIXPREC_KERNEL_MARKERS.get(kernel, 'o'),
                color=MIXPREC_COLOR_MAP.get(label, 'purple'),
                linestyle=MIXPREC_LINESTYLE_MAP.get(label, '-'),
                label=f"{kernel.replace('_', ' ').title()} ({label.replace('_', ' ')})",
...
save_plot("plots/multiprec_E_AB_over_ua")
plt.close()

# Fifth plot: E_{AB} with different normalizations for FP16 FP32 case only
plt.figure(figsize=FIGURE_SIZE)

uc=4.88e-04
ua=5.96e-08
label = "UC_FP16_UA_FP32"  # Only FP16 FP32 case
if label in data:
    kernels_data = data[label]
    ...
    for kernel, df in kernels_data.items():
        if not df.empty:
            ...
            # Plot E_AB/u_c
            beta_adhoc=np.log2(df['size']/32.0)*ua#+(32.0*uc)*np.log2(df['size']/32.0)*ua
            plt.plot(
                df['size'], df['error']/beta_adhoc,
                marker=MIXPREC_KERNEL_MARKERS.get(kernel, 'o'),
                color='blue',
                linestyle='--',
                label=r"$E_{AB} / (\log_2(k/32) \cdot u_a)$",
                markersize=MARKER_SIZE,
                linewidth=LINE_WIDTH,
            )

            # Plot E_AB/custom
            beta_ad2=np.log2(df['size']/32.0)*ua+df['size']*ua
            plt.plot(
                df['size'], df['error']/beta_ad2,
                marker=MIXPREC_KERNEL_MARKERS.get(kernel, 'o'),
                color='green',
                linestyle='--',
                label=r"$E_{AB} / custom$",
                markersize=MARKER_SIZE,
                linewidth=LINE_WIDTH,
            )
            ...
            plt.plot(
                df['size'], df['E_{AB}/beta'],
                marker=MIXPREC_KERNEL_MARKERS.get(kernel, 'o'),
                color=MIXPREC_COLOR_MAP.get(label, 'green'),
                linestyle='-.',
                label=f"{kernel.replace('_', ' ').title()} - E/beta",
                markersize=MARKER_SIZE,
                linewidth=LINE_WIDTH,
            )
# Add horizontal reference line at y=1
...
ax.set_xlabel("k - inner matrix dimension",
              fontsize=AXIS_LABEL_FONTSIZE,
              weight=get_axis_label_weight())
ax.set_ylabel(r"$E_{AB}$ (various normalizations)",
              fontsize=AXIS_LABEL_FONTSIZE,
              weight=get_axis_label_weight())
ax.set_yscale('log')
...
ax.set_title("FP16 Compute + FP32 Accumulate: Comparing Normalizations", fontsize=TITLE_FONTSIZE)
ax.grid(True, which='both', linestyle=GRID_LINESTYLE, alpha=GRID_ALPHA)

save_plot("plots/multiprec_E_AB_fp16fp32_comparison")
plt.close()
