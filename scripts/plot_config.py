...
# SAVE CONFIGURATION
# ============================================================================
SAVE_DPI = 600             # DPI for saved figures
...
Y_LIMITS_E_AB_OVER_BETA = {
    'uniform_positive': (1e-3, 1e+1),
    'wellcond': (1e-4, 1e+1),
    '2powers': (1e-3, 1e+1),
    'illcond': (1e-4, 1e+1),
    'zeromean': (1e-4, 1e+1)
...
def save_plot(base_filename, format=None):
    ...
    if fmt == "both":
        ...
        plt.savefig(f"{base_filename}.pdf", dpi=SAVE_DPI, bbox_inches=SAVE_BBOX)
    else:
