...
PLOT_TILED_VS_PAIRWISE_STD = False  # Tiled vs Pairwise standard deviation plots
PLOT_STD_METRICS = False             # E_AB std plots (part of basic metrics)
...
MATRIX_TYPES = ['uniform_positive', '2powers', 'illcond']  # Set to None for all, or list like ['2powers', 'wellcond']
...
def create_beta_plots(df, actual_matrix_sizes):
    ...
    for matrix_type in matrix_types:
        ...
        # Set y-axis limits (LAST, so nothing overrides it)
        if matrix_type in Y_LIMITS_E_AB_OVER_BETA:
            ax.set_ylim(Y_LIMITS_E_AB_OVER_BETA[matrix_type])
        else:
            ax.set_ylim(1e-2, 1e+1)  # Default limits
    ...
    for matrix_type in matrix_types:
        ...
        # Set y-axis limits (LAST, so nothing overrides it)
        if matrix_type in Y_LIMITS_E_AB:
            ax.set_ylim(Y_LIMITS_E_AB[matrix_type])
        else:
            ax.set_ylim(1e-2, 1e+1)  # Default limits
    ...
    if PLOT_STD_METRICS:
        ...
        print("\n=== Skipping std metric plots (PLOT_STD_METRICS=False) ===")
...
def main():
    ...
    if PLOT_TILED_VS_PAIRWISE_STD:
        print("\n=== Creating tiled vs pairwise std comparison ===")
        plot_tiled_vs_pairwise_std(df, actual_matrix_sizes)
    else:
        print("\n=== Skipping tiled vs pairwise std comparison (PLOT_TILED_VS_PAIRWISE_STD=False) ===")
