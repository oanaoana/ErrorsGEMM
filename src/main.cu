...
int main(int argc, char **argv) {
    ...
    if (is_mixprec_kernel(kernel_type)) {
        snprintf(g_data_folder, sizeof(g_data_folder), "data/tile%d/UC_%s_UA_%s",
                 TILE_SIZE, getComputeTypeString(), getAccumulateTypeString());
    } else {
        snprintf(g_data_folder, sizeof(g_data_folder), "data/tile%d/UC_UA_FP32", TILE_SIZE);
    }
