...
float compute_beta_factor(KernelType kernel_type, int K)
{
    ...
    const double u_c = unit_roundoff_compute_type();
    const double u_a = unit_roundoff_accumulate_type();

    // Use configuration flags from config.h
    bool include_cross_term = false;
    bool collapse_same_u = true;
    ...
    switch (kernel_type) {
        ...
        case KERNEL_TILED_MIXPREC: {                // Mixed precision tiled kernel
            ...
            tile_inner_k = TILE_SIZE;
            inter_pairwise     = false;
            include_cross_term = true;  // cross-terms in mixprec
            collapse_same_u = false;
            break;
        }
        case KERNEL_TILED_PAIRWISE_MIXPREC: {                // Mixed precision tiled pairwise kernel
            L            = ceil_div(K, TILE_SIZE);
            tile_inner_k = TILE_SIZE;
            inter_pairwise     = true;
            include_cross_term = true;  // cross-terms in mixprec
    ...
    //printf(" collapse_same_u = %d\n", collapse_same_u);
    //printf(" include_cross_term = %d\n", include_cross_term);
...
void run_multi_sample_analysis(MatrixType matrix_type, KernelType kernel_type, int n, int num_samples, bool reproducible) {
    ...
    double *frobenius_AB_error = (double*)malloc(num_samples * sizeof(double));
    ...
    for (int sample = 0; sample < num_samples; sample++) {
        ...
        frobenius_AB_error[sample] = sqrt(host_error_results[1]);
        // Compute beta normalized error: empirical_error / (|A||B|)
        normalized_errors[sample] = frobenius_errors[sample] / frobenius_AB_error[sample];
    ...
    ArrayStats norm_stats;
    compute_array_statistics(normalized_errors, num_samples, &norm_stats);

    // Compute log c_hat median
    const double u_accumulate = unit_roundoff_accumulate_type();  // Uses actual COMPUTE_TYPE
    const double u_compute = unit_roundoff_compute_type();
    ...
    printf("Average Error_AB/beta: %.6e\n", norm_stats.average/beta_factor);
    printf("Average Error_AB/u_c: %.6e\n", norm_stats.average/u_compute);
    printf("Average Error_AB/u_a: %.6e\n", norm_stats.average/u_accumulate);
    printf("Log c_hat median: %.6e\n", log_c_hat_median);

    printf("=== Unit Roundoff Values ===\n");
    printf("u_c (compute type): %.2e\n", unit_roundoff_compute_type());
    printf("u_a (accumulate type): %.2e\n", unit_roundoff_accumulate_type());
    ...
    if (fp) {
        ...
        fprintf(fp, "theoretical_beta,UC,UA,E_{AB}/beta,E_{AB}/u_c,E_{AB}/u_a,tile_size,log_c_hat_median\n");
        ...
        fprintf(fp, "%.16e,%.16e,%.16e,%.16e,%.16e,",
                norm_stats.average, norm_stats.std_dev, norm_stats.p10, norm_stats.p95, norm_stats.maximum);
        fprintf(fp, "%.16e,%s,%s,%.16e,%.16e,%.16e,%.16e\n",
                beta_factor,                     // theoretical_beta
                getComputeTypeString(),          // UC
                getAccumulateTypeString(),       // UA
                norm_stats.average/beta_factor,  // E_{AB}/beta
                norm_stats.average/u_compute,    // E_{AB}/u_c
                norm_stats.average/u_accumulate,  // E_{AB}/u_a
    ...
    free(frobenius_errors); free(frobenius_AB_error); free(normalized_errors);
