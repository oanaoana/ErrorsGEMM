...
void launch_cublas_tensor_fp16_fp32(const __half* d_Ah, const __half* d_Bh, float* d_Cf,
                                    int n) {
    cublasHandle_t handle;
    cublasCreate(&handle);

    // Enable TC path (older API); on newer CUDA, prefer cublasLt or per-op algo selection.
    cublasSetMathMode(handle, CUBLAS_TENSOR_OP_MATH);

    const float alpha = 1.0f, beta = 0.0f;

    // cuBLAS is column-major. This computes: C = alpha * B * A + beta * C
    // (so if your host code is row-major, either swap A/B or use transposes).
    cublasGemmEx(
        handle,
        CUBLAS_OP_N, CUBLAS_OP_N,
        n, n, n,
        &alpha,
        d_Bh, CUDA_R_16F, n,   // B in FP16
        d_Ah, CUDA_R_16F, n,   // A in FP16
        &beta,
        d_Cf, CUDA_R_32F, n,   // C in FP32 (accumulate in FP32)
        CUBLAS_COMPUTE_32F,    // compute type (FP32 accumulate on TC)
